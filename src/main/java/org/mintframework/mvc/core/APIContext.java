...
import org.mintframework.mvc.core.upload.MintUploadFile;
...
class APIContext {
	...
	APIContext(
		...
		for(int i=0 ;i<params.length ;i++){
			...
			} else if(clazz.equals(MintUploadFile.class) || clazz.equals(MintUploadFile[].class) || clazz.equals(File.class) || clazz.equals(File[].class)) { //获取文件上传配置
				isInitMulti = true;
				UploadConfig uinfo = param.getAnnotation(UploadConfig.class);
				UploadParamInfo upinfo = null;
				if(clazz.equals(MintUploadFile.class) || clazz.equals(File.class)) {
					upinfo = new UploadParamInfo(uinfo.limitSize(), i, 0);
				} else if(clazz.equals(MintUploadFile[].class) || clazz.equals(File[].class)) {
